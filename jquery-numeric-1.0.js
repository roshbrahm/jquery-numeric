...
* https://github.com/roshbrahm/jquery-numeric
*/
(function($) {
    $.fn.numeric = function(config, callback){
        ...
        var showVer=config.showver;
        if(showVer==undefined){
            showVer=false;
        }
        var togglePad=config.toggle;
        if(togglePad==undefined){
            togglePad=false;
        }
        var numpadId=$(this).attr("id");
        if(embedNumpad){
			var keypad="<br/><div class='numericKeypad'>";
            keypad+="<div class='row-pad'>" ;
            if(showVer){
                keypad+="<span style='font-size:8px;text-align:center;color:#fff;'>rNumPad v.1.0</span>"
            }
            if(togglePad){
                keypad+="<span class='togglePad_"+numpadId+" circle ' title='Toggle Numpad.'style='float:right;cursor:pointer;color:#fff;'><span class='arr-down'></span></span>";
            }
            keypad+="</div>"
            ...
            if(togglePad){
                $(".togglePad_"+numpadId).click(function(){
                    ...
                    if($(this).html()=='<span class="arr-down"></span>'){
                        console.log($(this).html())
                        $(this).html("<span class='arr-up'></span>");
                    }
                    else if($(this).html()=='<span class="arr-up"></span>'){
                        $(this).html("<span class='arr-down'></span>");
                    }
                });
            }
		}
    ...
    function negate(val,targetId,maxLen){
        ...
        if(caretPosition==0){
            if(elem.value.charAt(0)!="-"){
                ...
                else if(elem.value.length <=maxLen-1){
                    elem.value = val+elem.value;
                }
                setCaretPosition(targetId, caretPosition+1);
            }
